...
static PHP_INI_MH(OnUpdate_ini_path);

/* {{{ ini entries */
PHP_INI_BEGIN()
PHP_INI_ENTRY("hidef.ini_path",  (char*)default_ini_path,  PHP_INI_SYSTEM, OnUpdate_ini_path)
...
static int hidef_define_constant(char *key, zval *v, hidef_parser_ctxt* ctxt TSRMLS_DC)
{
	...
	//zend_error(E_NOTICE, "hidef_define_constant: %s", key);
...
static hidef_parser_ctxt g_ctxt = {0,};
static PHP_INI_MH(OnUpdate_ini_path) /* {{{ */
{
	long memory_limit = PG(memory_limit);
